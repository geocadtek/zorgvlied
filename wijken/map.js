					...
					if( content.features !=  "" )
					{
						...
						for( jj=0; jj<perceel_features.length; jj++ )
						{
							if( perceel_features[jj].attributes["ngsl"] != "0.0" )
							{
								...
								//}
								var tussen = ""
								if( perceel_features[jj].attributes["nvgsl"] == " " )
								{
									tussen = " "
								}
								else
    ...
    function open_PDF( nr_selectie )
    {
    	...
    	if( selectieLayer.features[nr_selectie].attributes["nvgsl"] == " " )
    	{
   	        tussen = " "
    	}
    	else
	...
	var settings_default_wfsserver = "https://geoweb.amstelveen.nl/geoserver/topp/wfs";
	var setting_default_namen = "topp:zorgvlied_fotos";
    ...
    {
        $('#zoeken_center').show();
        var strChar = jQuery.trim( $("#achternaam").attr('value') );
