	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
			...
			populate: function () { //get latest data
				...
				$.each(p.filter, function(i, item)
				{
					param.push ({name: 'filter['+i+'][id]', value : p.filter[i].id});
					param.push ({name: 'filter['+i+'][value]', value : p.filter[i].value});
				});
			...
			doSearch: function () {
				arrFilter=[];
				$.each($('select[id^="filter_"]'), function(i, item)
				{
					arrFilter.push ({id: $(item).attr("id"), value : ($(item).val()+'').replace(/,/g,', ')});
				
				});
				$.each($('input[id^="filter_"]'), function(i, item)
				{
					arrFilter.push ({id: $(item).attr("id"), value : $(item).attr("value")});
		...
		if (p.title)
		{
			...
			g.mDiv.innerHTML = '<div class="ftitle">'+p.title+'</div><div id="filter_block" class="filter"><div id="filter"></div><div class="fSearch" id="fSearch"><span></span></div></div>';
			$(g.gDiv).prepend(g.mDiv);
			$('#fSearch').click(function(e){g.doSearch()});
